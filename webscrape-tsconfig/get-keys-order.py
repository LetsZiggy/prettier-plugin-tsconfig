...
from helper import BColours, abs_path, init_webdriver, quote_string, t_grouped_level_order


def handle_quick_nav_elements(top_level_order: list[str], quick_nav_element: WebElement) -> list[str]:
	non_grouped_quick_nav_elements: list[WebElement] = quick_nav_element.find_elements(  # pyright: ignore[reportUnknownMemberType]
...
def handle_grouped_quick_nav_elements(
	...
	top_level_text = quote_string(quick_nav_element.find_element(By.CSS_SELECTOR, "h3 a").text)  # pyright: ignore[reportUnknownMemberType]
	if top_level_text not in top_level_order:
		top_level_order.append(top_level_text)

		grouped_level_order[top_level_text] = []
		category_level_elements: list[WebElement] = quick_nav_element.find_elements(  # pyright: ignore[reportUnknownMemberType]
			By.CSS_SELECTOR, "div.tsconfig-quick-nav-category"
		)
		for category_level_element in category_level_elements:
			category_name: str = quote_string(category_level_element.find_element(By.CSS_SELECTOR, "h4").text)  # pyright: ignore[reportUnknownMemberType]
			category_items: list[str] = [
				quote_string(category_item.text)
				for category_item in category_level_element.find_elements(By.CSS_SELECTOR, "ol li a")  # pyright: ignore[reportUnknownMemberType]
			]
...
def write_order_ts(top_level_order: list[str], grouped_level_order: t_grouped_level_order) -> None:
	...
	lines: list[str] = [
		"/* eslint-disable unicorn/single-line-block-comment-style */",
		"\n",
		"\n",
...
if __name__ == "__main__":
	...
	if len(args) == 0:
		...
		driver_path: str = abs_path(args[0] if args[0] != "" else "selenium-drivers/geckodriver")
